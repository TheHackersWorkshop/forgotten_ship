...
def generate_map(player_position, explored_coords, doors_coords):
    """
    Display the ship's map with the current player's position,
    explored areas, and visible doors.
    """
    ...
    # Plot explored cells
    for col_letter, row_num in explored_coords:
        x = cols.index(col_letter.upper())
        y = row_num - 1
        ship_map[y][x] = 'o'

    # Plot doors in explored areas only
    for coord in doors_coords.values():
        col_letter, row_num = coord
        if (col_letter, row_num) in explored_coords:
            x = cols.index(col_letter.upper())
            y = row_num - 1
            ship_map[y][x] = 'D'
